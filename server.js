...
app.post('/api/lobbies', (req, res) => {
    ...
    lobbies[lobbyId] = { type, hostId: null, gameState: 'LOBBY', players: {}, albums: [], roundTimer: null, shuffledPlayerIds: [], revealState: null };
...
app.ws('/draw/:lobbyId', (ws, req) => {
    ...
    ws.on('message', (msg) => {
        ...
        switch(data.type) {
            ...
            case 'submit_description': handleSubmitDescription(lobby, player, data.description, userId); break;
            case 'next_reveal_step': handleNextRevealStep(lobby, userId, true); break;
        }
    });
...
// --- Player Connection ---
function handlePlayerJoin(lobby, ws, userId, username) { if (lobby.gameState !== 'LOBBY') { ws.send(JSON.stringify({ type: 'error', message: 'Game has already started.' })); return ws.close(); } lobby.players[userId] = { username, ws, isDone: false }; if (!lobby.hostId) lobby.hostId = userId; ws.send(JSON.stringify({ type: 'initial_state', userId, hostId: lobby.hostId, players: getPlayerList(lobby) })); broadcast(lobby, { type: 'player_joined', player: { id: userId, username } }, ws); }
function handlePlayerLeave(lobby, userId) { if (!lobby.players[userId]) return; const wasHost = lobby.hostId === userId; const lobbyId = Object.keys(lobbies).find(key => lobbies[key] === lobby); delete lobby.players[userId]; if (Object.keys(lobby.players).length === 0) { if (lobby.roundTimer) clearTimeout(lobby.roundTimer); delete lobbies[lobbyId]; console.log(`Lobby ${lobbyId} deleted.`); } else { if (wasHost) lobby.hostId = Object.keys(lobby.players)[0]; checkRoundCompletion(lobby); broadcast(lobby, { type: 'player_left', userId, newHostId: lobby.hostId }); } }

// --- Game Logic ---
function handleStartGame(lobby, userId) { if (userId === lobby.hostId && lobby.gameState === 'LOBBY' && Object.keys(lobby.players).length >= 2) { lobby.gameState = 'PROMPTING'; lobby.shuffledPlayerIds = Object.keys(lobby.players).sort(() => 0.5 - Math.random()); broadcast(lobby, { type: 'game_started' }); } }
...
function handleSubmitDrawing(lobby, player, drawingCommands, userId) { if (lobby.gameState === 'DRAWING' && !player.isDone) { const album = lobby.albums.find(a => a.tasks[userId] === 'draw'); if (album) { album.steps.push({ type: 'drawing', author: player.username, content: drawingCommands }); player.isDone = true; checkRoundCompletion(lobby); } } }
function handleSubmitDescription(lobby, player, description, userId) { if (lobby.gameState === 'DESCRIBING' && !player.isDone) { const album = lobby.albums.find(a => a.tasks[userId] === 'describe'); if (album) { album.steps.push({ type: 'prompt', author: player.username, content: description }); player.isDone = true; checkRoundCompletion(lobby); } } }

// --- Round & Reveal Management ---
function checkRoundCompletion(lobby) { const playersInGame = Object.values(lobby.players); if (playersInGame.length === 0) return; const allDone = playersInGame.every(p => p.isDone); if (allDone) startNextRound(lobby); }
function startNextRound(lobby) {
    ...
    if (lobby.albums.length > 0 && lobby.albums[0].steps.length >= numPlayers) { startRevealPhase(lobby); return; }
    const isFirstRound = lobby.albums.length === 0;
    const nextStepIsDrawing = isFirstRound || lobby.albums[0].steps.length % 2 === 1;
    if (nextStepIsDrawing) startDrawingRound(lobby); else startDescribingRound(lobby);
}
...
function startDrawingRound(lobby) {
    ...
    if (lobby.albums.length === 0) { lobby.albums = playerIds.map(id => ({ originalAuthor: id, steps: [{ type: 'prompt', author: lobby.players[id].username, content: lobby.players[id].prompt }], tasks: {} })); }
    for (let i = 0; i < playerIds.length; i++) {
        ...
        assignedAlbum.tasks = {}; assignedAlbum.tasks[currentPlayerId] = 'draw';
    ...
    lobby.roundTimer = setTimeout(() => { Object.entries(lobby.players).forEach(([id, player]) => { if (!player.isDone) handleSubmitDrawing(lobby, player, [], id); }); }, ROUND_TIME);
}
...
function startDescribingRound(lobby) {
    ...
    for (let i = 0; i < playerIds.length; i++) {
        ...
        const assignedAlbumIndex = (i + roundNumber) % playerIds.length;
        const assignedAlbum = lobby.albums[assignedAlbumIndex];
        assignedAlbum.tasks = {}; assignedAlbum.tasks[currentPlayerId] = 'describe';
    ...
    lobby.roundTimer = setTimeout(() => { Object.entries(lobby.players).forEach(([id, player]) => { if (!player.isDone) handleSubmitDescription(lobby, player, "...", id); }); }, ROUND_TIME);
}
...
function startRevealPhase(lobby) {
    ...
    const finalAlbums = lobby.albums.map(album => ({ originalAuthorName: lobby.players[album.originalAuthor]?.username || 'A player', steps: album.steps }));
...
function handleNextRevealStep(lobby, userId, userForced = false) {
    ...
    lobby.roundTimer = setTimeout(() => { handleNextRevealStep(lobby, lobby.hostId, false); }, delay);
}
